        ...
        <div className={"relative group flex items-center hover:bg-black/5 p-4 transition w-full"}>
            <div className="group flex gap-x-2 items-start w-full">
                ...
                <div className="flex flex-col w-full">
                    ...
                    {isImage && (
                        <a href={fileUrl} target={"_blank"} rel={"noreferrer noopener"} className={"relative aspect-square rounded-md mt-2 overflow-hidden border flex items-center bg-secondary h-48 w-48"}>
                    ...
                    {isPDF && (
                            <div className={"relative flex items-center p-2 mt-2 rounded-md bg-background/10"}>
                                ...
                                <a href={fileUrl} target={"_blank"} rel={"noreferrer noopener"} className={"ml-2 text-sm text-indigo-500 dark:text-indigo-400 hover:underline"}>
                                    PDF file
                                </a>
                            </div>
                    )}
                    ...
                    {!fileUrl && isEditing && (
                        <Form {...form}>
                            <form className={"flex items-center w-full gap-x-2 pt-2"} onSubmit={form.handleSubmit(onClick)}>
                                <FormField control={form.control} name={"content"} render={( { field } ) => (
                                    <FormItem className={"flex-1"}>
                                        <FormControl>
                                            <div className="relative w-full">
                                                <Input disabled={isLoading} className={"p-2 bg-zinc-200/90 dark:bg-zinc-700/75 border-none border-0 focus-visible:ring-0 focus-visible:ring-offset-0 text-zinc-300 dark:text-zinc-200"} placeholder={"Edited messages"} {...field} />
                                            </div>
                                        </FormControl>
                                    </FormItem>
                                )} />
